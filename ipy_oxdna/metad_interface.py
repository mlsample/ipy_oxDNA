...
class oxDNARunner(mp.Process):
    index: int  # ?
    working_dir: str  # todo: pathlib
    input_file: str  # filename, apparnetly
    queue: mp.JoinableQueue

    _pconn: connection.Connection
    _cconn: connection.Connection
    _exception: Any

    manager: oxpy.OxpyManager

    def __init__(self, index: int, input_file: str, queue: mp.JoinableQueue):
        """
        Constructor
        """
...
class Estimator():
    ...
    # TODO: explainations of any of these
    base_dir: str
    A: float
    sigma: float
    tau: int
    ...
    def __init__(self,
        ...
        if use_seq_GPUs == True:
            nvidia_smi.nvmlInit()
            self.number_of_GPUs = nvidia_smi.nvmlDeviceGetCount()
            ...
            if not self.continue_run:
                ...
                for w in self.walkers:
                    ...
                    with open(os.path.join(w.working_dir, Estimator.INPUT_FILE), 'w+') as f:
                        if use_seq_GPUs:
                            input_file["CUDA_device"] = str(w.index % self.number_of_GPUs)
    ...
    def do_metadynamics_iteration(self, index):
        ...
        for w in self.walkers:
            if w.exception is not None:
                ...
                print(
                    f"The following error was raised during the execution of one of the child processes, aborting run:")
                print(error)
                print(traceback)
                self.stop_runners()
                exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dT", default=3,
                        help="Strength of the tempering (dT -> 0 leads to conventional simulations, dt -> infinity leads to conventional, non-well-tempered metadynamics)")
    parser.add_argument("--dim", default=1, help="Dimension of the order parameter")
    parser.add_argument("--p_fname", default="locs.meta",
                        help="File storing the indexes of the particles whose coordinates are used to build the order parameters")
    parser.add_argument("--ratio", default=0,
                        help="Use the angle defined from the ratio of the distances between centres of mass as the order parameter")
    parser.add_argument("--angle", default=0,
                        help="Use the angle defined from three centres of mass as the order parameter")
    ...
    parser.add_argument("--conf_interval", default=int(1e3),
                        help="Frequency with which configurations should be saved (in time steps)")
    ...
    parser.add_argument("--T", default=None,
                        help="The temperature at which the simulations will be run. If not set, the temperature in the initial input file will be used")
    parser.add_argument("--use_sequential_GPUs", action="store_true",
                        help="Each walker will try to use a dedicated GPU: the first one will attempt to use GPU 1, the second one GPU 2, etc.")
