...
class ComUmbrellaSampling(BaseUmbrellaSampling):
    ...
    def build_pre_equlibration_runs(self, simulation_manager, n_windows, com_list, ref_list, stiff, xmin, xmax,
                                    input_parameters, starting_r0, steps, observable=False, sequence_dependant=False,
                                    print_every=1e4, name='com_distance.txt', continue_run=False, protein=None,
                                    force_file=None):
        ...
        if continue_run is False:
            self.us_build.build(self.pre_equlibration_sims, input_parameters,
                                ...
                                observable=observable, sequence_dependant=sequence_dependant, protein=protein,
    ...
    def build_equlibration_runs(self, simulation_manager, n_windows, com_list, ref_list, stiff, xmin, xmax,
                                input_parameters,
                                observable=False, sequence_dependant=False, print_every=1e4, name='com_distance.txt',
                                continue_run=False,
    ...
    def build_production_runs(self, simulation_manager, n_windows, com_list, ref_list, stiff, xmin, xmax,
                              input_parameters,
                              observable=True, sequence_dependant=False, print_every=1e4, name='com_distance.txt',
                              continue_run=False,
                              protein=None, force_file=None):
        """
    ...
    def fig_ax(self):
        self.ax = self.wham.plt_fig(title='Free Energy Profile', xlabel='End-to-End Distance (nm)',
...
class MeltingUmbrellaSampling(ComUmbrellaSampling):
    ...
    def build_pre_equlibration_runs(self, simulation_manager, n_windows, com_list, ref_list, stiff, xmin, xmax,
                                    input_parameters, starting_r0, steps, observable=False, sequence_dependant=False,
                                    print_every=1e4, name='com_distance.txt', continue_run=False, protein=None,
                                    force_file=None, custom_observable=False, force_energy_split=True):
        self.observables_list = []
        self.windows.pre_equlibration_windows(n_windows)
        self.rate_umbrella_forces(com_list, ref_list, stiff, xmin, xmax, n_windows, starting_r0, steps)
    ...
    def discrete_and_continuous_converg_analysis(self, convergence_slice, temp_range, n_bins, xmin, xmax,
                                                 umbrella_stiff, max_hb, epsilon=1e-7, reread_files=False,
                                                 max_iterations=100000):
        self.wham_temp_interp_converg_analysis(convergence_slice, temp_range, n_bins, xmin, xmax, max_hb,
                                               umbrella_stiff, epsilon=epsilon, reread_files=reread_files,
                                               max_iterations=max_iterations)
        self.discrete_temp_interp_converg_analysis(convergence_slice, xmin, xmax, max_hb, temp_range,
                                                   reread_files=reread_files)

    def discrete_temp_interp_converg_analysis(self, convergence_slice, xmin, xmax, max_hb, temp_range,
                                              reread_files=False):
        ...
        for idx, converg_slice in enumerate(convergence_slice):
            ...
            free_energy, prob_discrete = self.temperature_interpolation(xmin, xmax, max_hb, temp_range,
                                                                        reread_files=reread_files,
                                                                        convergence_slice=converg_slice)
    ...
    def wham_temp_interp_converg_analysis(self, convergence_slice, temp_range, n_bins, xmin, xmax, umbrella_stiff,
                                          max_hb, epsilon=1e-7, reread_files=False, all_observables=False,
                                          max_iterations=100000):
        if (type(convergence_slice) == int) or (type(convergence_slice) == float):
            min_length = min([len(inner_list['com_distance']) for inner_list in self.obs_df])
            # split min_length into 3 slices
        ...
        for converg_slice in convergence_slice:
            free, F_i_temps, f_i_temps_over_time = self.wham_temperature_interpolation(temp_range, n_bins, xmin, xmax,
                                                                                       max_hb, umbrella_stiff,
                                                                                       epsilon=epsilon,
                                                                                       reread_files=reread_files,
                                                                                       max_iterations=max_iterations,
                                                                                       convergence_slice=converg_slice)
    ...
    def temperature_interpolation(self, xmin, xmax, max_hb, temp_range, reread_files=False, all_observables=False,
                                  convergence_slice=None, molcon=None):
        ...
        names = ['backbone', 'bonded_excluded_volume', 'stacking', 'nonbonded_excluded_volume', 'hydrogen_bonding',
                 'cross_stacking', 'coaxial_stacking', 'debye_huckel']
        ...
        if convergence_slice is not None:
            ...
            names = ['backbone', 'bonded_excluded_volume', 'stacking', 'nonbonded_excluded_volume', 'hydrogen_bonding',
                     'cross_stacking', 'coaxial_stacking', 'debye_huckel']
            truncated_potential_energy = [inner_list[names][convergence_slice] for inner_list in
                                          truncated_potential_energy]
            truncated_kinetic_energy = [inner_list[convergence_slice] for inner_list in truncated_kinetic_energy]
            truncated_force_energy = [inner_list[force_energy][convergence_slice] for inner_list in
                                      truncated_force_energy]
        ...
        top_file = self.production_sims[0].sim_files.top_filename
        ...
        energy_bias_per_window_per_temperature = np.array(
            self._new_calcualte_bias_energy(truncated_non_pot_energy, temp_range,
                                            truncated_potential_energy=truncated_potential_energy))

        energy_bias_per_window_per_temperature = [n_particles_in_system * innerlist for innerlist in
        ...
        beta_u_hb_list = [[[[] for _ in range(max_hb + 1)] for _ in range(len(temp_range))] for _ in
                          range(len(self.obs_df))]
        ...
        init_dat = self.production_sims[0].build_sim.last_conf_filename
    ...
    def _new_calcualte_bias_energy(self, umbrella_bias, temperature_range, truncated_potential_energy=None):
        ...
        for window_idx, (e_state_window, e_stack_window, e_ext) in enumerate(zip(e_state, e_stack, umbrella_bias)):
            ...
            et_old = (e_stack_window * STCK_FACT_EPS_OXDNA2) / (
                        STCK_BASE_EPS_OXDNA2 + old_temperature * STCK_FACT_EPS_OXDNA2)
            ...
            for temp_idx, new_temp in enumerate(new_temperatures):
                ...
                et_new = (e_stack_window * STCK_FACT_EPS_OXDNA2) / (
                            STCK_BASE_EPS_OXDNA2 + new_temp * STCK_FACT_EPS_OXDNA2)
                ...
                energy_term = -(e0_old + e_ext + new_temp * et_old) / new_temp + (
                            e0_old + e_ext + old_temperature * et_old) / old_temperature
    ...
    def wham_temperature_interpolation(self, temp_range, n_bins, xmin, xmax, umbrella_stiff, max_hb, epsilon=1e-7,
                                       reread_files=False, all_observables=False, max_iterations=100000,
                                       convergence_slice=None):
        ...
        truncated_kinetic_energy = [inner_list['kinetic_energy'][:min_length] for inner_list in self.obs_df]
        truncated_force_energy = [inner_list[force_energy][:min_length] for inner_list in self.obs_df]
        truncated_umbrella_bias = [0.5 * np.double(umbrella_stiff) * (com_values - eq_pos) ** 2 for com_values, eq_pos
                                   in zip(truncated_com_values, x_range)]
        ...
        if convergence_slice is not None:
            ...
            truncated_force_energy = [inner_list[force_energy][convergence_slice] for inner_list in
                                      truncated_force_energy]
            truncated_umbrella_bias = [0.5 * np.double(umbrella_stiff) * (com_values - eq_pos) ** 2 for
                                       com_values, eq_pos in zip(truncated_com_values, x_range)]
        # w_i = self.weight_sample()
        ...
        new_energy_per_window = self._new_calcualte_bias_energy(truncated_non_pot_energy, temp_range,
                                                                truncated_potential_energy=truncated_potential_energy)
        ...
        weights_in_bins = [[[[] for _ in range(len(bin_edges) - 1)] for _ in range(len(self.obs_df))] for _ in
                           range(len(temp_range))]
            ...
            for window_idx in range(n_windows):
                for bin_idx in range(n_bins):
                    if len(weights_in_bins[temp_idx][window_idx][bin_idx]) > 0:
                        log_histogram[temp_idx, window_idx, bin_idx] = logsumexp(
                            weights_in_bins[temp_idx][window_idx][bin_idx])
                    else:
        ...
        f_i_temps_old = np.array(
            [[rng.normal(loc=0.0, scale=1.0, size=None) for _ in range(len(self.obs_df))] for _ in temp_range_scaled])
        ...
        with tqdm(desc='WHAM', leave=True, bar_format=custom_bar_format) as pbar:
            while ((first is True) or (np.max(np.abs(f_i_temps_new - f_i_temps_old)) > epsilon)) and (
    ...
    def _weight_sample_pre_compute_input(self, truncated_com_distance, truncated_potential_energy,
                                         truncated_force_energy, beta, n_particles_in_system, n_restraints):
    ...
    def weight_sample(self, epsilon=1e-15, reread_files=False, convergence_slice=None):
        ...
        biasing_function, log_biasing_function, all_window_energies, all_pot_energies, all_force_energies = self._weight_sample_pre_compute_input(
            truncated_com_distance, truncated_potential_energy, truncated_force_energy, beta, n_particles_in_system,
            n_restraints)
        ...
        with tqdm(desc='Non-parametric WHAM', leave=True, bar_format=custom_bar_format) as pbar:
            while convergence_criterion > epsilon:
                ...
                log_rho_new = np.log(1) - logsumexp(log_biasing_function + log_f_i_old, b=num_samples_per_window,
                                                    axis=1)
    ...
    def calculate_melting_temperature(self, temp_range):
        ...
        p0 = [max(self.inverted_finfs), np.median(temp_range), 1,
              min(self.inverted_finfs)]  # initial guesses for L, x0, k, b
...
class PymbarAnalysis:
    ...
    def run_mbar_fes(self, reread_files=False, sim_type='prod', uncorrelated_samples=False, restraints=False,
                     force_energy_split=False):

        if uncorrelated_samples is False:
            u_kln, N_k = self.setup_input_data(reread_files=reread_files, sim_type=sim_type, restraints=restraints,
                                               force_energy_split=force_energy_split)
        elif uncorrelated_samples is True:
            u_kln, N_k = self.setup_input_data_with_uncorrelated_samples(reread_files=reread_files, sim_type=sim_type,
        ...
        self.basefes = pymbar.FES(u_kln, N_k, mbar_options=mbar_options, verbose=True,
                                  timings=True)  # , mbar_options=mbar_options)
    ...
    def _setup_temp_scaled_potential(self, N_max, temp_range):
        ...
        new_energy_per_window = self.base_umbrella._new_calcualte_bias_energy(non_pot_energy, temp_range,
                                                                              truncated_potential_energy=u_kn)
        new_energy_per_window = [val.swapaxes(1, 0) for val in new_energy_per_window]
        new_energy_per_window = np.array(
            [np.pad(inner_list, ((0, N_max - len(inner_list)), (0, 0)), 'constant') for inner_list in
             new_energy_per_window])
    ...
    def _setup_restrain_potential(self, com_kn, N_k, N_max, force_energy_split):
        if force_energy_split is True:
            ...
            umbrella_potentials_kn = [self.base_umbrella.w_i(com_n_k[:N_k_k], r0,
                                                             self.base_umbrella.stiff) * 2 / self.base_umbrella.n_particles_in_system
    ...
    def n_hb_fes_hist(self, max_hb, uncorrelated_samples=False, temp_range=None):
        ...
        hb_list_kn = np.array(
            [np.pad(inner_list, (0, N_max - len(inner_list)), 'constant') for inner_list in hb_list_kn])
    ...
    def hb_contact_fes_hist(self, n_bins=200, uncorrelated_samples=False, temp_range=None):
        ...
        hb_contacts_kn = np.array(
            [np.pad(inner_list, (0, N_max - len(inner_list)), 'constant') for inner_list in hb_contacts_kn])
...
def use_splits_for_error_bars(hbs, coms, contacts, p_value):
    ...
    for i in range(len(mean_metrics)):
        ...
        for j in range(len(mean_metrics[i])):
            plt.errorbar(bin_metrics[i], mean_metrics[i][j], yerr=sem_metrics[i][j], label=f'{j}', fmt='.-',
                         capsize=2.5, capthick=1, errorevery=1)
...
def plot_melting_curve(us, metric, max_hbs, monomer_conc=None, magnesium_conc=None, ax=None, plot=True):
    ...
    def setup_input_data_with_uncorrelated_samples(self, reread_files=False, sim_type='prod', restraints=False,
                                                   force_energy_split=False):
        if reread_files is False:
            if self.base_umbrella.obs_df == None:
                self.base_umbrella.analysis.read_all_observables(sim_type=sim_type)
        ...
        for k in range(K):
            t0, g, Neff_max = pymbar.timeseries.detect_equilibration_binary_search(com_kn[k, :N_k[k]])
            indices = pymbar.timeseries.subsample_correlated_data(com_kn[k, t0:N_k[k]], g=g)
...
class UmbrellaBuild:
    ...
    def prepare_simulation_environment(self, sims):
        ...
        if exists(join(self.base_umbrella.system_dir, umbrella_stage)) and bool(
                os.listdir(join(self.base_umbrella.system_dir, umbrella_stage))):
            if self.base_umbrella.clean_build is True:
                answer = input(
                    'Are you sure you want to delete all simulation files? Type y/yes to continue or anything else to return use UmbrellaSampling(clean_build=str(force) to skip this message')
    ...
    def modify_topology_for_unique_pairing(self):
        ...
        for sim in self.base_umbrella.equlibration_sims:
            topology_file_path = sim.sim_files.top_filename  # Assuming this is the absolute path to the topology file
...
class UmbrellaInfoUtils:
    ...
    def get_n_particles_in_system(self):
        top_file = self.base.equlibration_sims[0].sim_files.top_filename
        with open(top_file, 'r') as f:
...
class UmbrellaProgress:
    ...
    def read_pre_equlibration_progress(self):
        if exists(join(self.base.system_dir, 'pre_equlibration')):
            ...
            for window in range(n_windows):
                self.base.pre_equlibration_sims.append(Simulation(join(self.base.pre_equlibration_sim_dir, str(window)),
                                                                  join(self.base.pre_equlibration_sim_dir,
                                                                       str(window))))
    ...
    def read_equlibration_progress(self):
        if exists(join(self.base.system_dir, 'equlibration')):
            ...
            for window in range(self.base.n_windows):
                self.base.equlibration_sims.append(Simulation(join(self.base.equlibration_sim_dir, str(window)),
                                                              join(self.base.equlibration_sim_dir, str(window))))
    ...
    def read_production_progress(self):
        if exists(join(self.base.system_dir, 'production')):
            ...
            self.base.production_window_dirs = [join(self.base.production_sim_dir, str(window)) for window in
                                                range(n_windows)]

            self.base.production_sims = []
            for s, window_dir, window in zip(self.base.equlibration_sims, self.base.production_window_dirs,
                                             range(n_windows)):
                self.base.production_sims.append(
                    Simulation(self.base.equlibration_sims[window].sim_dir, str(window_dir)))
    ...
    def read_wham_progress(self):
        if exists(join(self.base.system_dir, 'production', 'com_dir', 'freefile')):
            ...
            try:
                self.base.standard_error, self.base.confidence_interval = self.base.wham.bootstrap_w_mean_error(
                    self.base.free)
            except:
    ...
    def read_convergence_analysis_progress(self):
        if exists(join(self.base.system_dir, 'production', 'com_dir', 'convergence_dir')):
            try:
                ...
                self.base.chunk_convergence_analysis_dir = join(self.base.convergence_dir,
                                                                'chunk_convergence_analysis_dir')
                self.base.data_truncated_convergence_analysis_dir = join(self.base.convergence_dir,
                                                                         'data_truncated_convergence_analysis_dir')
                self.base.wham.chunk_dirs = [join(self.base.chunk_convergence_analysis_dir, chunk_dir) for chunk_dir in
                                             os.listdir(self.base.chunk_convergence_analysis_dir)]
                self.base.wham.data_truncated_dirs = [join(self.data_truncated_convergence_analysis_dir, chunk_dir) for
                                                      chunk_dir in
                                                      os.listdir(self.base.data_truncated_convergence_analysis_dir)]

                self.base.chunk_dirs_free = [self.base.wham.to_si(self.base.n_bins, chunk_dir) for chunk_dir in
                                             self.base.wham.chunk_dirs]
                self.base.chunk_dirs_mean = [self.base.wham.w_mean(free_energy) for free_energy in
                                             self.base.chunk_dirs_free]
                try:
                    self.base.chunk_dirs_standard_error, self.base.chunk_dirs_confidence_interval = zip(
                        *[self.base.wham.bootstrap_w_mean_error(free_energy) for free_energy in
                          self.base.chunk_dirs_free]
                    )
                except:
                ...
                self.base.data_truncated_free = [self.base.wham.to_si(self.base.n_bins, chunk_dir) for chunk_dir in
                                                 self.base.wham.data_truncated_dirs]
                self.base.data_truncated_mean = [self.base.wham.w_mean(free_energy) for free_energy in
                                                 self.base.data_truncated_free]
                try:
                    self.base.data_truncated_standard_error, self.data_truncated_confidence_interval = zip(
                        *[self.base.wham.bootstrap_w_mean_error(free_energy) for free_energy in
                          self.data_truncated_free]
                    )
                except:
                    self.base.data_truncated_standard_error = ['failed' for _ in
                                                               range(len(self.base.data_truncated_free))]
                    self.base.data_truncated_confidence_interval = ['failed' for _ in
                                                                    range(len(self.base.data_truncated_free))]
            except:
                pass
...
class UmbrellaAnalysis:
    ...
    def view_observable(self, sim_type, idx, sliding_window=False, observable=None):
        ...
        if sim_type == 'pre_eq':
            self.base_umbrella.pre_equlibration_sims[idx].analysis.plot_observable(observable, fig=False,
                                                                                   sliding_window=sliding_window)

        if sim_type == 'eq':
            self.base_umbrella.equlibration_sims[idx].analysis.plot_observable(observable, fig=False,
                                                                               sliding_window=sliding_window)

        if sim_type == 'prod':
            self.base_umbrella.production_sims[idx].analysis.plot_observable(observable, fig=False,
                                                                             sliding_window=sliding_window)
    ...
    def view_cms(self, sim_type, idx, xmax, print_every, sliding_window=10, fig=True):
        if sim_type == 'eq':
            self.base_umbrella.equlibration_sims[idx].analysis.view_cms_obs(xmax, print_every,
                                                                            sliding_window=sliding_window, fig=fig)
        if sim_type == 'prod':
            self.base_umbrella.production_sims[idx].analysis.view_cms_obs(xmax, print_every,
                                                                          sliding_window=sliding_window, fig=fig)
...
class UmbrellaWindow:
    ...
    def pre_equlibration_windows(self, n_windows):
        ...
        self.base_umbrella.pre_equlibration_sims = [
            Simulation(self.base_umbrella.file_dir, join(self.base_umbrella.pre_equlibration_sim_dir, str(window))) for
            window in range(n_windows)]
    ...
    def equlibration_windows(self, n_windows):
        ...
        if hasattr(self.base_umbrella, 'pre_equlibration_sim_dir'):
            self.base_umbrella.equlibration_sims = [
                Simulation(sim.sim_dir, join(self.base_umbrella.equlibration_sim_dir, sim.sim_dir.split('/')[-1])) for
                sim in self.base_umbrella.pre_equlibration_sims]

        else:
            self.base_umbrella.equlibration_sims = [
                Simulation(self.base_umbrella.file_dir, join(self.base_umbrella.equlibration_sim_dir, str(window))) for
                window in range(n_windows)]
    ...
    def production_windows(self, n_windows):
        ...
        self.base_umbrella.production_window_dirs = [join(self.base_umbrella.production_sim_dir, str(window)) for window
                                                     in range(n_windows)]
        self.base_umbrella.production_sims = []
        for s, window_dir, window in zip(self.base_umbrella.equlibration_sims,
                                         self.base_umbrella.production_window_dirs, range(n_windows)):
            self.base_umbrella.production_sims.append(
                Simulation(self.base_umbrella.equlibration_sims[window].sim_dir, str(window_dir)))
...
class WhamAnalysis:
    ...
    def convergence_analysis(self, n_chunks, data_added_per_iteration, wham_dir, xmin, xmax, umbrella_stiff, n_bins,
                             tol, n_boot):
        """
        ...
        self.base_umbrella.chunk_convergence_analysis_dir = join(self.base_umbrella.convergence_dir,
                                                                 'chunk_convergence_analysis_dir')
        self.base_umbrella.data_truncated_convergence_analysis_dir = join(self.base_umbrella.convergence_dir,
                                                                          'data_truncated_convergence_analysis_dir')
        ...
        self.base_umbrella.chunk_dirs_mean = [self.w_mean(free_energy) for free_energy in
                                              self.base_umbrella.chunk_dirs_free]
        try:
            ...
            self.base_umbrella.chunk_dirs_standard_error = ['failed' for _ in
                                                            range(len(self.base_umbrella.chunk_dirs_free))]
            self.base_umbrella.chunk_dirs_confidence_interval = ['failed' for _ in
                                                                 range(len(self.base_umbrella.chunk_dirs_free))]

        self.data_truncated_convergence_analysis(data_added_per_iteration)

        self.base_umbrella.data_truncated_free = [self.to_si(self.n_bins, chunk_dir) for chunk_dir in
                                                  self.data_truncated_dirs]
        self.base_umbrella.data_truncated_mean = [self.w_mean(free_energy) for free_energy in
                                                  self.base_umbrella.data_truncated_free]
        try:
            ...
            self.base_umbrella.data_truncated_standard_error = ['failed' for _ in
                                                                range(len(self.base_umbrella.data_truncated_free))]
            self.base_umbrella.data_truncated_confidence_interval = ['failed' for _ in
                                                                     range(len(self.base_umbrella.data_truncated_free))]
        ...
        for idx, df in enumerate(self.base_umbrella.chunk_dirs_free):
            ...
            try:
                ax.errorbar(df.loc[:, '#Coor'], df.loc[:, 'Free'],
                            ...
                            label=f'{label} {indicator[0]:.2f} nm \u00B1 {indicator[1]:.2f} nm')
    ...
    def plot_truncated_free_energy(self, ax=None, title='Free Energy Profile', label=None, errorevery=1):
        ...
        for idx, df in enumerate(self.base_umbrella.data_truncated_free):
            ...
            indicator = [self.base_umbrella.data_truncated_mean[idx],
                         self.base_umbrella.data_truncated_standard_error[idx]]
            try:
                ax.errorbar(df.loc[:, '#Coor'], df.loc[:, 'Free'],
                            yerr=df.loc[:, '+/-'], capsize=2.5, capthick=1.2,
                            linewidth=1.5, errorevery=errorevery,
    ...
    def plot_free_energy_mod(self, negative=False, ax=None, title='Free Energy Profile', c='black', label=None,
                             errorevery=15):
        if ax is None:
            ax = self.plt_fig()
        if label is None:
